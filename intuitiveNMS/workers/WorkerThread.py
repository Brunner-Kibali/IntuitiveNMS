...
class WorkerThread(threading.Thread):
    def __init__(self, intuitivenms, worker_type, connection_type, serial_no, heartbeat):

        threading.Thread.__init__(self)

        self.intuitivenms = intuitivenms
    ...
    def run(self):
        ...
        print(f"{self.worker_type} worker: starting work, intuitiveNMS={self.intuitivenms}")
    ...
    def heartbeat(self, interval):
        ...
        while True and not self.terminate:
            ...
            try:
                start = time.time()
                rsp = requests.post(
                    "http://" + self.intuitivenms + ":5000/worker/heartbeat", json=heartbeat_info
                )
                ...
                print(f"{str(datetime.now())[:-3]}: --- {self.worker_type} error connecting to intuitiveNMS server: {e}")
    ...
    def process_command(self, worker_info):
        ...
        if self.worker_type == "capture":
            worker_thread = CaptureThread(self.intuitivenms, self.serial_no, worker_info)
        elif self.worker_type == "portscan":
            worker_thread = PortscanThread(self.intuitivenms, self.serial_no, worker_info)
        elif self.worker_type == "traceroute":
            worker_thread = TracerouteThread(self.intuitivenms, self.serial_no, worker_info)
        else:
