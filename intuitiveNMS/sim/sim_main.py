...
parser.add_argument('-intuitiveNMS',  default='localhost', help='IP address of intuitiveNMS server')
args = parser.parse_args()
intuitiveNMS_ip = args.intuitiveNMS
print(f"IntuitiveNMS IP: {intuitiveNMS_ip}")
...
while True:
    ...
    for device in devices:
        ...
        try:
            start = time.time()
            rsp = requests.post(
                "http://" + intuitiveNMS_ip + ":5000/device/heartbeat", json=heartbeat_info
            )
            ...
            print(f"Error in connecting to intuitiveNMS server: {repr(e)}")
